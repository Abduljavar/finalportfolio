...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["create"])) {
        ...
        $stmt = $conn->prepare("INSERT INTO content (title, date, description, file_photo, system_file) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $title, $date, $description, $photo_name, $system_file_name);
        $stmt->execute();
        $stmt->close();
        ...
        $stmt = $conn->prepare("UPDATE content SET title=?, date=?, description=? WHERE id=?");
        $stmt->bind_param("sssi", $title, $date, $description, $content_id);
        $stmt->execute();
        $stmt->close();
